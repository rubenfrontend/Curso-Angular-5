...
import { AfterViewChecked, Component, Input, Output, EventEmitter, OnInit, ViewChild,
 forwardRef, AfterViewInit, ElementRef, Renderer2, HostListener } from '@angular/core';
...
export class CompleterComponent implements OnInit, ControlValueAccessor, AfterViewChecked, AfterViewInit {
  ...
  displayNoResults: any = true;
   ...
   @HostListener('focusout', ['$event']) onfocusout(event: any) {
     if (event.target.value === '' && event.target.nextElementSibling.classList.contains('mdb-autocomplete-clear')) {
       this.renderer.setStyle(event.target.nextElementSibling, 'visibility', 'hidden');
     }
   }
